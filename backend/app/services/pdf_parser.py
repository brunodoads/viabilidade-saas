...
_VISION_SYSTEM_PROMPT = (
    ...
    "- raw_name: APENAS a descricao comercial do produto — o que apareceria num anuncio do Mercado Livre. "
    "NAO inclua codigos internos, referencias de estoque ou numeracoes de catalogo no raw_name. "
    "Exemplos corretos: 'Luva Vinil Descartavel P', 'Frasco Plastico 500ml c/ Tampa', 'Caixa Papelao 50x30cm'. "
    "Exemplos ERRADOS (nao faca isso): '10001 - Luva Vinil P', 'A-045 Frasco Plastico', 'REF: B23 Caixa'.\n"
    "- cost: QUALQUER valor monetario visivel associado ao produto - pode ser "
    "preco, preco de venda, valor, vlr, vl, preco de tabela, atacado, custo ou "
    "qualquer campo numerico com R$. Use o primeiro valor monetario que encontrar. "
    "Se houver multiplos precos, use o MENOR. Null apenas se nao houver nenhum numero.\n"
    "- sku: codigo interno, referencia, cod, ref, SKU do produto. "
    "IMPORTANTE: se o nome do produto comeca com um codigo numerico (ex: '10001 -', '8875 ') "
    "ou alfanumerico (ex: 'A-045', 'REF-123', 'COD: B23'), coloque esse codigo aqui no sku, "
    "NAO no raw_name. (opcional, null se nao encontrar)\n"
    ...
    "7. raw_name deve comecar com a descricao do produto, nunca com codigo ou numero isolado\n\n"
    "Exemplo de entrada: linha com '10001 - LUVA VINIL P CX100  R$ 8,50'\n"
    "Exemplo de saida correta:\n"
    '[{"raw_name": "Luva Vinil P", "cost": 8.50, "sku": "10001", '
    '"category": null, "supplier": null}]\n\n'
    "Outro exemplo: '8875 ESCOVA DENTE ADULTO MACIA PCT12  12,90'\n"
    '[{"raw_name": "Escova Dente Adulto Macia", "cost": 12.90, "sku": "8875", '
    '"category": null, "supplier": null}]'
)
...
def _try_claude_vision(file_path: Path) -> "ParseResult":
    ...
    if not settings.CLAUDE_API_KEY:
        result.add_error(
            "CLAUDE_API_KEY nao configurada - Claude Vision nao disponivel."
        )
        result.confidence = ParseConfidence.FAILED
        return result
    ...
    if not page_images:
        result.add_error(
            "Nao foi possivel converter paginas do PDF em imagens (PyMuPDF necessario)."
        )
        result.confidence = ParseConfidence.FAILED
        return result
    ...
    if len(page_images) == MAX_VISION_PAGES:
        result.add_warning(
            "PDF tem muitas paginas - processando apenas as primeiras {} "
            "para controlar custo.".format(MAX_VISION_PAGES)
        )
