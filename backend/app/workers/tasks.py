...
def process_catalog_task(self: Task, catalog_id_str: str) -> dict:
    ...
    try:
        ...
        # O worker roda em container SEPARADO — filesystems independentes.
        # Solução MVP: file_content está no DB. Se arquivo não existir localmente,
        ...
        if not _file_path.exists() and catalog.file_content:
            ...
            logger.info("Arquivo materializado do DB → %s | catalog_id=%s", _temp_file.name, catalog_id)
        elif not _file_path.exists() and not catalog.file_content:
            raise FileNotFoundError(
                f"Arquivo não encontrado em disco ({catalog.file_path}) e "
                f"file_content não está no banco. Re-faça o upload."
            )
        ...
        try:
            ...
            logger.info(
                "PARSING %s | %d produtos | confiança=%s",
        ...
        logger.info("Pipeline CONCLUÍDO | catalog_id=%s | %d produtos", catalog_id, len(products))
        ...
        db.close()
        # Limpar arquivo temporário se foi criado neste processamento
