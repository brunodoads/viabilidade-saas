...
def search_listings_batch(
    ...
    NOTA: o actor karamelo aceita apenas `keyword` singular. O campo `keywords`
    (lista) e silenciosamente ignorado -> retorna 200 com 0 itens -> `_fetch_batch()`
    detecta e aciona `_fetch_batch_fallback()` com chamadas individuais.
...
def _fetch_batch(queries, api_token, actor_id, max_pages, batch_idx, total_batches):
    ...
    # Se batch retornou 0 itens para N queries, o actor ignorou 'keywords'
    # (retorna 200 vazio em vez de 400). Fallback automatico para chamadas individuais.
    if total_items == 0 and len(queries) > 0:
        logger.warning(
            "Apify ML Batch [%d/%d]: 0 itens para %d queries (200 vazio) -- "
            "actor nao suporta keywords array. Usando fallback individual.",
            batch_idx + 1, total_batches, len(queries)
        )
        return _fetch_batch_fallback(queries, api_token, actor_id, max_pages)
